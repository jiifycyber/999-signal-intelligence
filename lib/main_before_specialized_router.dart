...
import 'modules/trading_module_panel.dart';
...
class _NexusDashboardState extends State<NexusDashboard> {
  ...
  void openPanel(String title) {
    ...
    showDialog(
      context: context,
      builder: (_) => TradingModulePanel(
        title: title,
        pair: selectedPair,
        price: currentPriceText(),
        direction: signal?.directionText ?? 'WAIT',
        confidence: signal?.confidence ?? 0.0,
        trend: signal?.trend ?? 'Analyzing',
        momentum: signal?.momentum ?? 'Analyzing',
        setup: signal?.setup ?? 'Waiting for qualified setup',
        entry: fmt(signal?.entry),
        stopLoss: fmt(signal?.stopLoss),
        tp1: fmt(signal?.takeProfit1),
        tp2: fmt(signal?.takeProfit2),
        tp3: fmt(signal?.takeProfit3),
      ),
    );
  }
